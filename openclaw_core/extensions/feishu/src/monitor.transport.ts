...
export async function monitorWebSocket({
  ...
  const staggerMs = account.config.reconnectDelayMs ?? 0;
  if (staggerMs > 0) {
    log(`feishu[${accountId}]: ws startup delay ${staggerMs}ms (reconnectDelayMs)`);
    await delayWithAbort(staggerMs, abortSignal);
  }
